...
if __name__ == '__main__':
    ...
    matrices = [
        ...
        # Over-abundant rectangle #1
        np.array([[400, 150, 400],
          [400, 450, 600]#,
          # [300, 225, 300],
          # [350, 250, 400]
          ])
    ...
    for cost_matrix in matrices:
        indexes, manipulated_matrix = m.compute(cost_matrix)
        print('\nRESULTS\n')
        total_cost = 0
        for r, c in indexes:
            if r >= len(cost_matrix):
                print('mapped padded row to column {}'.format(c))
            elif c >= len(cost_matrix[0]):
                print('mapped row {} to padded column'.format(r))
            else:
                x = cost_matrix[r][c]
                total_cost += x
                print(('(%d, %d) -> %d' % (r, c, x)))
